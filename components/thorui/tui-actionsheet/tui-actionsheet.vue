		...
		<view class="tui-actionsheet" :class="{'tui-actionsheet-show':show,'tui-actionsheet-radius':radius}">
			...
			<view :class="[isCancel?'tui-operate-box':'']">
				<block v-for="(item,index) in itemList" :key="index">
					<view class="tui-actionsheet-btn tui-actionsheet-divider" v-if="!item.noShow" :class="{'tui-btn-last':!isCancel && index==itemList.length-1}"
					 hover-class="tui-actionsheet-hover" :hover-stay-time="150" :data-index="index" :style="{color:item.color || '#2B2B2B'}"
					 @tap="handleClickItem">
						{{item.text}}
						<text v-if="item.redDot" class="redDot"></text>
						<template v-if="item.type == 'isHave'">
							<text v-if="item.val" class="isHaveIcon iconfont2 icon-duihao"></text>
							<text v-else class="isHaveIcon iconfont2 icon-shanchu1"></text>
						</template>
					 </view>
				</block>
			</view>
