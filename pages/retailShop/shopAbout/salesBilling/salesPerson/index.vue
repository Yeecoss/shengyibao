...
<view class="salesBilling">
  <!-- 自定义标题栏 -->
  <navigationBar title="销售员" id="top"></navigationBar>
  <view class="dis-flex search-input-box">
    <view class="search-input">
      <view class="dis-flex flex-y-center search-box">
          ...
          <input v-model="keywords" @submit="search()" @blur="search()" class="input" placeholder="搜索销售员" placeholderStyle="color:#aba9a9;font-size:24rpx;" type="text"/>
      </view>
    </view>
  </view>
  <scroll-view :refresher-threshold="20" refresher-enabled :refresher-triggered="triggered" @refresherrefresh="refresherrefresh" @scrolltolower="bindDownLoad" scroll-y :style="'height: ' + scrollHeight  + 'px;'">
    <div class="listBox">
      <template v-if="list.length > 0">
        <div class="listItem" v-for="(item, index) in list" :key="index" @tap="checkItem(item)">
          <div class="name">{{item.real_name}}</div>
        </div>
        <view v-if="no_more" class="no-more f-24 col-9">生意宝技术支持～</view>
      </template>
...
export default {
  ...
  methods: {
    checkItem(item) {
      ...
      if (prevPage.$vm.checkSalesPerson) {
        prevPage.$vm.checkSalesPerson(item)
      }
      uni.navigateBack();
    },
    search() {
    ...
    getListData(next) {
      ...
      App._get('store.shop.clerk/getClerkList', obj, (result) => {
        if (result.code == '1') {
          if (next) {
            next ? next() : ''
          }
    ...
    add() {
      uni.navigateTo({
        url: '/pages/retailShop/shopAbout/salesBilling/salesPerson/add/index'
      });
    },
    setListHeight() {
