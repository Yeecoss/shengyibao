...
<view class="salesBilling">
  <!-- 自定义标题栏 -->
  <navigationBar title="仓库" id="top"></navigationBar>
  <view class="dis-flex search-input-box">
    <view class="search-input">
      <view class="dis-flex flex-y-center search-box">
          ...
          <input v-model="keywords" @submit="search()" @blur="search()" class="input" placeholder="搜索仓库" placeholderStyle="color:#aba9a9;font-size:24rpx;" type="text"/>
      </view>
    </view>
  </view>
...
export default {
  ...
  methods: {
    checkItem(item) {
      ...
      if (prevPage.$vm.checkWareHouse) {
        prevPage.$vm.checkWareHouse(item)
      }
      uni.navigateBack();
    },
    search() {
    ...
    getListData(next) {
      ...
      App._get('store.shop.warehouse/getWarehouseList', obj, (result) => {
        if (result.code == '1') {
          if (next) {
            next ? next() : ''
          }
    ...
    add() {
      uni.navigateTo({
        url: '/pages/retailShop/shopAbout/salesBilling/wareHouse/add/index'
      });
    },
    setListHeight() {
